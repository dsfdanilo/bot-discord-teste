...
module.exports = async ({ client, inter, queue }) => {
    if (!queue || !queue.isPlaying()) return inter.editReply({ content: `Não há nenhuma música tocando... tentar novamente ? ❌`, ephemeral: true });
    ...
    try {
        ...
        if (!song) return inter.editReply({ content: `A letra da música não foi encontrada ${queue.currentTrack.title}... tentar novamente ? ❌`, ephemeral: true });
        ...
        for (let i = 0; i < lyrics.length; i += 4096) {
            ...
            embeds.push(new EmbedBuilder()
                ...
                .setColor('#2f3136')
                .setTimestamp()
                .setFooter({ text: 'Aqui esá a letra da música!', iconURL: inter.member.avatarURL({ dynamic: true }) })
            );
        }
